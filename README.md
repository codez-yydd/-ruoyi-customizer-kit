...
- **替换后台 UI** — 可选 Vben Admin（Element Plus，pnpm monorepo）或 Arco Design（Arco Design Vue，npm 单包）预置模板整体替换原 `ruoyi-ui`，标题 / 端口 / 版权经占位符自动写入
...
| enable_config_rewrite | boolean | `true` | 重写 application 配置为 yaml 三件套 |
...
| db_name | string | `""` | 新数据库名；空则用 `new_module_prefix` |
...
| enable_oss | boolean | `false` | 是否引入 OSS 对象存储 |
...
| enable_jwt | boolean | `false` | 是否定制 JWT 配置 |
...
| server_port | number | `8080` | 后端服务端口 |
| server_name | string | `""` | 对外域名；空则用 `localhost` |
| use_https | boolean | `false` | 是否启用 HTTPS（生成证书占位段） |
| enable_startup_scripts | boolean | `false` | 是否生成启动/停止脚本（`.sh` + `.bat`） |
| enable_replace_ui | boolean | `false` | 是否用预置后台模板替换原 `ruoyi-ui`（仅 ruoyi-vue） |
