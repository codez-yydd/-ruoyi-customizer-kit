...
/** 立即执行一次：PUT /monitor/job/run（后端 @RequestBody SysJob，需带 jobId/jobGroup） */
export function runJobOnce(jobId: number, jobGroup?: string): Promise<void> {
  return request.put('/monitor/job/run', { jobId, jobGroup })
}

/** 导出定时任务：POST /monitor/job/export */
export function exportJob(query: JobQuery, fileName = '定时任务.xlsx'): Promise<void> {
  return exportRequest('/monitor/job/export', query, fileName)
}
...
/** 调度日志分页列表：GET /monitor/jobLog/list */
export function listJobLog(query: JobLogQuery): Promise<PageResult<SysJobLog>> {
  return request.get<PageResult<SysJobLog>, PageResult<SysJobLog>>('/monitor/jobLog/list', {
    params: query
  })
}

/** 批量删除调度日志：DELETE /monitor/jobLog/{jobLogIds}（多个逗号拼接） */
export function delJobLog(jobLogIds: number | string | Array<number | string>): Promise<void> {
  return request.delete(`/monitor/jobLog/${jobLogIds}`)
}

/** 清空调度日志：DELETE /monitor/jobLog/clean */
export function cleanJobLog(): Promise<void> {
  return request.delete('/monitor/jobLog/clean')
}

/** 导出调度日志：POST /monitor/jobLog/export */
export function exportJobLog(query: JobLogQuery, fileName = '调度日志.xlsx'): Promise<void> {
  return exportRequest('/monitor/jobLog/export', query, fileName)
}
