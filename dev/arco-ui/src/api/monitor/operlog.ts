...
/** 操作日志分页列表：GET /monitor/operlog/list */
export function listOperlog(query: OperLogQuery): Promise<PageResult<SysOperLog>> {
  return request.get<PageResult<SysOperLog>, PageResult<SysOperLog>>('/monitor/operlog/list', {
    params: query
  })
}

/** 批量删除操作日志：DELETE /monitor/operlog/{operIds}（多个逗号拼接） */
export function delOperlog(operIds: number | string | Array<number | string>): Promise<void> {
  return request.delete(`/monitor/operlog/${operIds}`)
}

/** 清空操作日志：DELETE /monitor/operlog/clean */
export function cleanOperlog(): Promise<void> {
  return request.delete('/monitor/operlog/clean')
}

/** 导出操作日志：POST /monitor/operlog/export */
export function exportOperlog(query: OperLogQuery, fileName = '操作日志.xlsx'): Promise<void> {
  return exportRequest('/monitor/operlog/export', query, fileName)
}
