-- ----------------------------
-- RuoYi-Vue PostgreSQL 初始化脚本
-- 由 ry_20260417.sql 按锻造台方言规则手工校对转换。
-- 用户需先自建库，本脚本不含 CREATE DATABASE / USE。
-- 表块之间保留 -- ---------------------------- 分隔，供清除 quartz 逻辑切分。
-- ----------------------------
...
create table sys_notice_read (
  read_id int8 not null generated by default as identity,
...
-- ----------------------------
-- IDENTITY 序列推进（显式 INSERT 不会更新序列，对齐 MySQL AUTO_INCREMENT）
-- ----------------------------
SELECT setval(pg_get_serial_sequence('sys_dept', 'dept_id'), GREATEST((SELECT MAX(dept_id) FROM sys_dept), 200));
SELECT setval(pg_get_serial_sequence('sys_user', 'user_id'), GREATEST((SELECT MAX(user_id) FROM sys_user), 100));
SELECT setval(pg_get_serial_sequence('sys_role', 'role_id'), GREATEST((SELECT MAX(role_id) FROM sys_role), 100));
SELECT setval(pg_get_serial_sequence('sys_post', 'post_id'), GREATEST((SELECT MAX(post_id) FROM sys_post), 100));
SELECT setval(pg_get_serial_sequence('sys_config', 'config_id'), GREATEST((SELECT MAX(config_id) FROM sys_config), 100));
SELECT setval(pg_get_serial_sequence('sys_dict_type', 'dict_id'), GREATEST((SELECT MAX(dict_id) FROM sys_dict_type), 100));
SELECT setval(pg_get_serial_sequence('sys_dict_data', 'dict_code'), GREATEST((SELECT MAX(dict_code) FROM sys_dict_data), 100));
SELECT setval(pg_get_serial_sequence('sys_menu', 'menu_id'), GREATEST((SELECT MAX(menu_id) FROM sys_menu), 2000));
SELECT setval(pg_get_serial_sequence('sys_job', 'job_id'), GREATEST((SELECT MAX(job_id) FROM sys_job), 100));
SELECT setval(pg_get_serial_sequence('sys_notice', 'notice_id'), GREATEST((SELECT MAX(notice_id) FROM sys_notice), 10));