 ...
 * Cloud 网关 Path=/system/** StripPrefix=1，接口走 /system/online
 */
...
export function listOnline(query: {
  ...
  return requestClient.get<TableResult<SysUserOnline>>('/system/online/list', {
    params: query,
  });
}
...
export function forceLogout(tokenId: string) {
  return requestClient.delete(
    `/system/online/${encodeURIComponent(tokenId)}`,
  );
}
