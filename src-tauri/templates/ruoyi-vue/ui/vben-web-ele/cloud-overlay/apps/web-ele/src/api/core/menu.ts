...
function transformRuoYiMenu(
  ...
  return menus.map((menu) => {
    ...
    // 外链新窗口：meta.link 仅保留 http(s)。
    // 内嵌 InnerLink：只用 iframeSrc，避免 generateMenus 用 link 覆盖 path 后误开新窗。
    // 再兜底过滤非 http，避免菜单 path 被写成相对段导致 router.push 乱跳。
    const rawLink = external ? menu.path : isInnerLink ? undefined : meta?.link;
    const finalLink =
      rawLink && isExternalLink(rawLink) ? rawLink : undefined;
    const iframeSrc = isInnerLink ? meta?.link : undefined;

    // 一级菜单 frame 时后端 name 可能为空串，vue-router 同名会冲突
    const routeName =
      menu.name && String(menu.name).trim()
        ? menu.name
        : `Route${finalPath.replace(/[^\w]+/g, '_')}`;
...
/**
 * 获取用户所有菜单（适配Cloud GET /system/menu/getRouters）
 *
...
export async function getAllMenusApi() {
  const raw = await requestClient.get<RuoYiRouter[]>('/system/menu/getRouters');
