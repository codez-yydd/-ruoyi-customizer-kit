...
/**
 * 代码生成 API（对应 Cloud GenController /gen，网关 Path=/code/**）
 */
...
export function listTable(query: Record<string, any>) {
  return requestClient.get<TableResult<GenTable>>('/code/gen/list', {
    params: query,
  });
}
...
export function listDbTable(query: Record<string, any>) {
  return requestClient.get<TableResult<GenTable>>('/code/gen/db/list', {
    params: query,
  });
}
...
export function updateGenTable(data: Partial<GenTable>) {
  return requestClient.put('/code/gen', data);
}
...
export function importTable(data: { tables: string; tplWebType: string }) {
  return requestClient.post('/code/gen/importTable', undefined, {
    params: data,
    rawResponse: true,
  });
}
...
export function createTable(data: { sql: string; tplWebType: string }) {
  return requestClient.post('/code/gen/createTable', undefined, {
    params: data,
    rawResponse: true,
  });
}
...
export function previewTable(tableId: number) {
  return requestClient.get<Record<string, string>>(
    `/code/gen/preview/${tableId}`,
  );
}

/** 删除已导入的表配置 */
export function delTable(tableId: number | number[] | string) {
  return requestClient.delete(`/code/gen/${tableId}`);
}

/** 生成代码到自定义路径 */
export function genCode(tableName: string) {
  return requestClient.get(`/code/gen/genCode/${tableName}`);
}

/** 同步数据库表结构 */
export function synchDb(tableName: string) {
  return requestClient.get(`/code/gen/synchDb/${tableName}`);
}
...
export function downloadBatchGenCode(tables: string) {
  return requestClient.get('/code/gen/batchGenCode', {
    params: { tables },
    responseType: 'blob',
  });
}
