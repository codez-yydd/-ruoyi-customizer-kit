...
/** 业务表字段行（GET /code/gen/{tableId} 的 rows） */
...
/** GET /code/gen/{tableId} 响应 data（实测：{info, rows, tables}） */
...
/** GET /code/gen/preview/{tableId} 响应 data：模板路径 -> 代码内容 */
export type GenPreviewResult = Record<string, string>

/** 业务表分页列表：GET /code/gen/list（Cloud 网关 Path=/code/** → gen 服务 /gen） */
export function listGen(query: GenQuery): Promise<PageResult<GenTable>> {
  return request.get<PageResult<GenTable>, PageResult<GenTable>>('/code/gen/list', {
    params: query
  })
}

/** 业务表详情（含字段列表）：GET /code/gen/{tableId} */
export function getGenTable(tableId: number): Promise<GenDetailResult> {
  return request.get<GenDetailResult, GenDetailResult>(`/code/gen/${tableId}`)
}

/** 待导入库表分页列表：GET /code/gen/db/list */
export function listGenDb(query: GenQuery): Promise<PageResult<GenTable>> {
  return request.get<PageResult<GenTable>, PageResult<GenTable>>('/code/gen/db/list', {
    params: query
  })
}

/**
 * 导入业务表：POST /code/gen/importTable
 * 后端为 @RequestParam 形式，tables 多个逗号拼接
 */
export function importGenTable(tables: string, tplWebType: string): Promise<void> {
  return request.post('/code/gen/importTable', undefined, {
    params: { tables, tplWebType }
  })
}

/** 批量删除业务表：DELETE /code/gen/{tableIds}（多个逗号拼接） */
export function delGenTable(tableIds: number | string | Array<number | string>): Promise<void> {
  return request.delete(`/code/gen/${tableIds}`)
}

/** 同步表结构：GET /code/gen/synchDb/{tableName} */
export function synchGenTable(tableName: string): Promise<void> {
  return request.get(`/code/gen/synchDb/${encodeURIComponent(tableName)}`)
}

/** 生成预览：GET /code/gen/preview/{tableId} */
export function previewGenTable(tableId: number): Promise<GenPreviewResult> {
  return request.get<GenPreviewResult, GenPreviewResult>(`/code/gen/preview/${tableId}`)
}

/** 生成代码下载（zip 流）：GET /code/gen/download/{tableName} */
export async function downloadGenCode(tableName: string, fileName = 'ruoyi.zip'): Promise<void> {
  ...
  const response = await request.get<unknown, AxiosResponse<Blob>>(`/code/gen/download/${encodeURIComponent(tableName)}`, {
    responseType: 'blob'
  })
